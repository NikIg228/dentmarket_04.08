...
**Последнее точечное обновление:** 2026-09-03, UI-миграция через commits `a24c734`, `f35367a` и текущий Admin integration operations slice
...
| Admin UX                            | `CODED`         | Shared shell, organization dialog, foundation controls, catalog workflows, supplier decision controls, platform assurance, supplier-data operations, priority work queue, agreement signing, integration detail и connector readiness используют общий `Dm*`-контракт; supplier-data IA перенесена в «Загрузка товаров», а order blockers и ЭЦП — в «Заказы и договоры»; Admin 10/10 tests, typecheck/build и production-browser QA на 390/768/1024/1280 px пройдены с детерминированным API stub; responsive layout, dynamic provider fields, action feedback и keyboard focus проверены | Trust и commerce/settings legacy-панели ещё не мигрированы; полный live daily workflow не доказан | Продолжать связанными operator slices, затем выполнить `verify:web` с реальным API и pilot-данными |
