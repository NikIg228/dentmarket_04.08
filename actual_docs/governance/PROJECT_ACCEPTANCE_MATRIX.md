...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: API 176, schemas 39, api-client 7, Buyer 8, Supplier 4; `verify:rate-limit-auth`, `verify:outbox`, `verify:pilot-backend`, `verify:outbound-security`, `verify:web` проходят | Admin/Landing UI без unit coverage; B4.6 ещё не пройден | Следующая задача — B4.6 |
