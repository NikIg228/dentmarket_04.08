...
**Последнее точечное обновление:** 2026-09-03, UI-миграция через commit `a24c734` и текущий Admin supplier/order operations slice
...
| Admin UX                            | `CODED`         | Shared shell, organization dialog, foundation controls, catalog workflows, supplier decision controls, platform assurance, supplier-data operations, priority work queue и agreement signing используют общий `Dm*`-контракт; supplier-data IA перенесена в «Загрузка товаров», а order blockers и ЭЦП — в «Заказы и договоры»; Admin 8/8 tests, typecheck/build и production-browser QA на 390/768/1024/1280 px пройдены с детерминированным API stub; action feedback и keyboard focus проверены | Integration detail, trust, commerce/settings legacy-панели ещё не мигрированы; полный live daily workflow не доказан | Продолжать связанными operator slices, затем выполнить `verify:web` с реальным API и pilot-данными |
