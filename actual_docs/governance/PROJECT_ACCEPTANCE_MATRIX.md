...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1 добавляет metrics, B4.2 доказывает запуск API на restored PostgreSQL | Внешняя доставка alert и dashboard ещё не имеют `LIVE_VERIFIED` | Следующая задача B4.5 — security/dependency scan |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 117 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `verify:web` — 17/17, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 3/3; PostgreSQL, B4.1 и B4.2 gates проходят | Admin/Landing UI без unit coverage; B4.3–B4.6 ещё не пройдены | B4.5 закрыть раньше B4.3 из-за high dependency backlog |
...
Следующая реализационная задача — **B4.5: security/dependency scan**. B4.2
закрыт реальным изолированным PostgreSQL restore, data/object reconciliation,
Prisma migration status, API health/readiness и CI gate. B4.5 поднят выше B4.3,
потому что audit уже подтвердил 15 high и 6 moderate уязвимостей. Managed
WAL/PITR, S3 retention и production snapshot restore остаются deployment
evidence и не получают `LIVE_VERIFIED` от локального drill.
