...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1 добавляет защищённый Prometheus endpoint и PostgreSQL metrics для критических flows | Внешняя доставка alert и dashboard ещё не имеют `LIVE_VERIFIED` | Следующая задача B4.2 — backup/restore rehearsal |
...
| Dependency security      | `BLOCKED`              | `pnpm audit --prod --audit-level high` запущен 2026-08-19; новый `prom-client` не входит в найденные vulnerable paths | 21 существующая уязвимость: 15 high и 6 moderate в Next/sharp/pdfjs/Prisma и транзитивных пакетах | B4.5: обновить и совместно перепроверить dependency graph, build, PDF/import и web flows |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 117 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `pnpm verify:web` — 17/17, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 3/3, B3.2 и B3.3 repeat — 10/10 каждый; PostgreSQL и B4.1 gates проходят | Admin/Landing UI без unit coverage; B4.2–B4.6 ещё не пройдены | Добавлять детерминированный gate на каждый следующий эксплуатационный риск |
...
Следующая реализационная задача — **B4.2: backup/restore rehearsal**. B4.1
закрыт защищённым Prometheus endpoint, PostgreSQL metrics, versioned alert rules,
runbook и `pnpm verify:observability`. Внешняя доставка alert остаётся deployment
evidence, а production auth, Redis, object storage, ClamAV и внешние интеграции —
отдельными production-readiness gates.
