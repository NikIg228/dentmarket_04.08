...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1–B4.2, dependency remediation, B4.5-R1, R2A, R2B, R2C и R2D проверены                                                                         | Source findings закрыты, но внешний alert/dashboard и повторный security regression не имеют `LIVE_VERIFIED`              | Следующая задача B4.5-R2E — повторный security regression                                  |
...
| Application security      | `BLOCKED`              | Dependency finding, 4 High и все 4 Medium source findings закрыты; R2A/R2B/R2C/R2D regressions и полный gate stack зелёные                                      | Повторный полный security regression и live infrastructure evidence ещё не зафиксированы       | B4.5-R2E: повторить source-to-sink validation и закрыть B4.5                    |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 150 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `verify:outbound-security` — 25/25 + webhook static gate; последовательный `verify:web` — 17/17; PostgreSQL, security и runtime gates проходят                  | Admin/Landing UI без unit coverage; B4.3–B4.4 и B4.6 ещё не пройдены; B4.5-R2E ещё не закрыт | Сначала B4.5-R2E, затем B4.3                                      |
...
Следующая реализационная задача — **B4.5-R2E: повторный security regression**.
B4.5-R2A, R2B, R2C и R2D закрыли organization enumeration/capability disclosure,
XLSX decompression exhaustion, delayed session revocation и notification webhook
SSRF; focused regressions, полный gate stack и 17/17 browser regression прошли.
