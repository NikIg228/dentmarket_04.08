...
**Последнее точечное обновление:** 2026-09-03, UI-миграция через commits `a24c734`, `f35367a`, `e25cd97` и текущий Admin trust operations slice
...
| Admin UX                            | `CODED`         | Shared shell, organization dialog, foundation controls, catalog workflows, supplier decision controls, platform assurance, supplier-data operations, priority work queue, agreement signing, integration detail, connector readiness и trust operations используют общий `Dm*`-контракт; supplier-data IA перенесена в «Загрузка товаров», а order blockers и ЭЦП — в «Заказы и договоры»; trust-раздел получает поставщиков из API и отделяет внутренний операторский сигнал от публичного trust score вне пилота; Admin 12/12 tests, typecheck/build и production-browser QA на 390/768/1024/1280 px пройдены с детерминированным API stub; responsive layout, actions, feedback и empty state проверены | Commerce/settings legacy-панели ещё не мигрированы; полный live daily workflow не доказан | Завершить settings slice, затем выполнить `verify:web` с реальным API и pilot-данными |
