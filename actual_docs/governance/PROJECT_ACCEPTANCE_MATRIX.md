...
| Автоматические тесты      | `E2E_VERIFIED`         | 2026-09-03: `npm test` — 14/14 Turbo-задач; API 189, schemas 45, api-client 7, Buyer 14, Supplier 9, shared UI 3, Admin 18 и Landing 3 unit-теста. `npm run verify:web` после UI-миграции проходит 17/17 сценариев с локальными PostgreSQL/API и production web bundles | Component coverage части рабочих панелей остаётся ограниченным; live Google/Apple, реальная ЭЦП и ручная accessibility matrix не входят в этот suite | Сохранять `verify:web` как pilot regression gate и добавлять сценарий на каждый новый критический workflow |
| Typecheck и build         | `UNIT_VERIFIED`        | 2026-09-03: `npm run typecheck` — 15/15 Turbo-задач; production builds Admin, Buyer, Supplier и Landing проходят на Next 16.2.11                                                                                  | Полный root build после closeout не повторялся; Next предупреждает о deprecated middleware convention     | Сделать merge-gate и отдельно оптимизировать Buyer bundle/build                                         |
...
- [x] 2026-09-03 `npm run typecheck` проходит: 15/15 Turbo-задач.
- [x] 2026-09-03 `npm test` проходит: 14/14 Turbo-задач; API 189, schemas 45, api-client 7, Buyer 14, Supplier 9, shared UI 3, Admin 18 и Landing 3 теста.
