...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6 проходят; Flow A, B2.1–B2.3 и B3.1–B3.2 проверяют checkout, supplier execution, документы и CSV review/publication в PostgreSQL | Откат ошибочного import batch без потери raw/evidence ещё не доказан | Следующая задача B3.3 — безопасный rollback batch |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 113 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `pnpm verify:web` — 16/16, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 2/2 и B3.2 repeat 10/10; PostgreSQL gate проходит | Admin/Landing UI без unit coverage; B3.3 ещё не пройден | Добавлять по одному детерминированному E2E на следующий критический journey |
| Typecheck и build         | `UNIT_VERIFIED`        | `pnpm typecheck`: 12/12 задач; `pnpm build`: 8/8 задач, API и четыре web-приложения собраны                                                         | Общая сборка заняла около 5 минут; Next предупреждает о deprecated middleware convention | Сделать merge-gate и отдельно оптимизировать Buyer bundle/build                       |
...
| Импорт CSV                          | `E2E_VERIFIED`  | B3.1 проводит UTF-8 CSV через upload/quarantine/parser и matching; B3.2 доказывает operator review, materialization, publication и Buyer search; PostgreSQL и Playwright проверяют tenant isolation, stale version, idempotency и zero residue | Rollback ошибочного batch остаётся B3.3 | Перейти к B3.3, не расширяя gate до неподтверждённых форматов |
...
| Создание предложения                | `E2E_VERIFIED`  | B3.2 материализует из import row product/variant/packaging, DRAFT offer, price и inventory; после публикации Buyer видит ту же карточку | Изменение/откат ошибочно опубликованного batch остаётся B3.3                         | Сохранить Flow B3 и добавить compensating rollback                           |
...
| Checkout и разбиение по поставщикам | `E2E_VERIFIED`  | Flow A создаёт заказы одному и двум поставщикам; DB проверяет checkout/orders/reservations | Production payment остаётся отдельным этапом                                        | Сохранять Flow A при реализации B3.3                                        |
| Подтверждение заказа поставщиком    | `E2E_VERIFIED`  | `verify:flow-b2` 4/4: full/partial confirmation, shipment и order document pack, tenant 403, stale 409, totals, audit/outbox и Buyer visibility | Partial release внешнего резерва ждёт connector orchestration                         | Сохранять Flow B2 при реализации B3.3                                    |
...
Статус core catalog slice Flow B: **`E2E_VERIFIED`** (`pnpm verify:flow-b3`, 2/2;
B3.2 repeat 10/10). Регистрация и production onboarding поставщика остаются
...
- [x] `pnpm verify:web` открывает landing, Buyer, Supplier и Admin и проходит 16/16 browser-сценариев без критических ошибок.
...
Следующая реализационная задача — **B3.3: доказать безопасный откат ошибочного import batch без потери raw/evidence**. Flow A, полный Flow C (B2.1–B2.3) и B3.1–B3.2 закрыты локальными E2E, но production auth, Redis, production object storage, ClamAV и внешние интеграции остаются отдельными production-readiness gates.
