...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1–B4.2 и dependency-часть B4.5 проверены                                                                                                                 | Scan B4.5 оставил 4 high и 4 medium source findings; внешняя доставка alert/dashboard не имеет `LIVE_VERIFIED`               | Следующая задача B4.5-R1 — high source remediation                                                      |
...
| Application security      | `BLOCKED`              | Standard scan `7a5358c7-a6f3-459d-a1fa-8bc22ef5c822`: 5 high/4 medium на исходном revision; dependency high закрыт и отдельно перепроверен                                                                     | Остаются 4 high и 4 medium source findings: authority, BOLA, SSRF, tenant disclosure, session revocation и parser exhaustion | B4.5-R1: закрыть 4 high, затем повторить source-to-sink validation и решить medium backlog              |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 117 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; последовательный `verify:web` — 17/17, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 3/3; PostgreSQL, security и runtime gates проходят          | Admin/Landing UI без unit coverage; B4.3–B4.4 и B4.6 ещё не пройдены; 4 high/4 medium app findings не исправлены             | Сначала B4.5-R1, затем повторный security scan/validation                                               |
