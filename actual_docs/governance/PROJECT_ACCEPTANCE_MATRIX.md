...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6 проходят; Flow A, B2.1–B2.3 и B3.1–B3.3 проверяют checkout, supplier execution, документы и полный CSV lifecycle в PostgreSQL | Production observability и alert response ещё не доказаны | Следующая задача B4.1 — metrics и alerts |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 113 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `pnpm verify:web` — 17/17, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 3/3, B3.2 и B3.3 repeat — 10/10 каждый; PostgreSQL gate проходит | Admin/Landing UI без unit coverage; B4 ещё не пройден | Добавлять по одному детерминированному E2E на следующий критический journey |
...
| Создание предложения                | `E2E_VERIFIED`  | B3.2 материализует из import row product/variant/packaging, DRAFT offer, price и inventory; B3.3 безопасно скрывает и архивирует batch-owned effect | Live supplier update cadence и connector SLA не доказаны                         | Сохранять Flow B3; следующий risk gate — production observability                           |
...
Следующая реализационная задача — **B4.1: metrics и alerts для критических API, worker/outbox, checkout и import rollback**. Flow A, полный Flow C (B2.1–B2.3) и B3.1–B3.3 закрыты локальными E2E, но production auth, Redis, production object storage, ClamAV и внешние интеграции остаются отдельными production-readiness gates.
