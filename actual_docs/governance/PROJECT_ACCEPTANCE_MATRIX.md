...
| Admin UX                            | `CODED`         | Shared shell, organization dialog, foundation controls, product corrections, catalog quality и B3.2 import review используют общий `Dm*`-контракт; Admin 4/4 tests, typecheck/build и production-browser QA на 390/768/1024/1280 px пройдены с детерминированным API stub | Supplier/commerce/integration/operations legacy-панели ещё не мигрированы; полный live daily workflow не доказан | Продолжать связанными operator slices, затем выполнить `verify:web` с реальным API и pilot-данными |
