...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1–B4.2, dependency remediation, B4.5-R1, R2A и R2B проверены                                                                                     | Остаются 2 Medium source findings; внешняя доставка alert/dashboard не имеет `LIVE_VERIFIED`              | Следующая задача B4.5-R2C — delayed session revocation                                  |
...
| Application security      | `BLOCKED`              | Dependency finding, 4 High, Medium organization disclosure и XLSX exhaustion закрыты; R2A/R2B regressions и полный gate stack зелёные                                      | Остаются 2 Medium: session revocation и notification SSRF       | B4.5-R2C: закрыть delayed session revocation и повторить source-to-sink validation                    |
