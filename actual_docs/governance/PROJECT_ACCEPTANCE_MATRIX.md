...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6 проходят; Flow A, B2.1–B2.3 и B3.1 проверяют checkout, supplier execution, документы и CSV staging/validation/matching в PostgreSQL | Operator review, publication и Buyer visibility импорта ещё не доказаны | Следующая задача B3.2 — operator review → publication → Buyer visibility |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 113 API, 38 schema, 6 api-client, 5 Buyer и 1 Supplier; `pnpm verify:web` — 15/15, Flow A — 2/2, Flow B2 — 4/4, Flow B3 — 1/1 и repeat 10/10; PostgreSQL gate проходит | Admin/Landing UI без unit coverage; B3.2 ещё не пройден | Добавлять по одному детерминированному E2E на следующий критический journey |
...
| Импорт CSV                          | `E2E_VERIFIED`  | B3.1 проводит реальный UTF-8 CSV через upload/quarantine/parser, raw staging, validation и matching; PostgreSQL и Playwright проверяют tenant isolation, idempotency и zero residue | Operator review/publication остаются B3.2, rollback — B3.3 | Перейти к B3.2, не расширяя CSV gate до неподтверждённых форматов |
...
| Checkout и разбиение по поставщикам | `E2E_VERIFIED`  | Flow A создаёт заказы одному и двум поставщикам; DB проверяет checkout/orders/reservations | Production payment остаётся отдельным этапом                                        | Сохранять Flow A при реализации B3.2                                        |
| Подтверждение заказа поставщиком    | `E2E_VERIFIED`  | `verify:flow-b2` 4/4: full/partial confirmation, shipment и order document pack, tenant 403, stale 409, totals, audit/outbox и Buyer visibility | Partial release внешнего резерва ждёт connector orchestration                         | Сохранять Flow B2 при реализации B3.2                                    |
...
- [x] `pnpm verify:web` открывает landing, Buyer, Supplier и Admin и проходит 15/15 browser-сценариев без критических ошибок.
...
Следующая реализационная задача — **B3.2: доказать operator review → publication → Buyer visibility через PostgreSQL + Playwright**. Flow A, полный Flow C (B2.1–B2.3) и B3.1 CSV staging/validation/matching закрыты локальными E2E, но production auth, Redis, production object storage, ClamAV и внешние интеграции остаются отдельными production-readiness gates.
