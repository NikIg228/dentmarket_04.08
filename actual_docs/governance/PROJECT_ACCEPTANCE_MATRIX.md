...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1–B4.2, dependency remediation и B4.5-R1 High remediation проверены                                                                                      | Остаются 4 Medium source findings; внешняя доставка alert/dashboard не имеет `LIVE_VERIFIED`              | Следующая задача B4.5-R2A — organization enumeration remediation                                        |
...
| Application security      | `BLOCKED`              | Dependency finding и 4 High source findings закрыты; R1B diff-scan `657c3363-632e-42c0-8d08-f09880a55745` проверил 9/9 source items с complete coverage и `0 findings`                                         | Остаются 4 Medium: organization disclosure, XLSX exhaustion, session revocation и notification SSRF       | B4.5-R2: закрыть Medium отдельными change sets и повторить source-to-sink validation                    |
...
| Автоматические тесты      | `E2E_VERIFIED`         | `pnpm test`: 136 API, 38 schema, 7 api-client, 5 Buyer и 1 Supplier; `verify:outbound-security` — 25/25; последовательный `verify:web` — 17/17; PostgreSQL, security и runtime gates проходят                  | Admin/Landing UI без unit coverage; B4.3–B4.4 и B4.6 ещё не пройдены; 4 Medium app findings не исправлены | Сначала B4.5-R2A, затем остальные Medium и повторный security scan                                      |
...
| Admin UX                            | `CODED`         | B3.2 import review queue доказана production-browser gate; остальные операторские панели существуют                                                                                                                                                                   | Нет принятого сквозного ежедневного workflow для всех P0/P1 задач                           | Расширять по одной проверенной очереди, не переписывать кабинет целиком                       |
...
Следующая реализационная задача — **B4.5-R2A: remediation organization
enumeration/capability disclosure**. B4.5-R1A закрыла три authorization High,
а R1B — integration SSRF; diff-scan R1B проверил 9/9 source items с complete
coverage и `0 findings`, полный gate stack, включая PostgreSQL и 17/17 browser
regression, прошёл. Проект остаётся `BLOCKED` по Application Security из-за
четырёх Medium findings. B4.3 начинается после их source-to-sink remediation и
повторного security regression; live infrastructure evidence также остаётся
явно открытым.
