...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.3 и B3.1–B3.3 проходят; B4.1–B4.2, dependency remediation, B4.5-R1 и B4.5-R2A проверены                                                                                      | Остаются 3 Medium source findings; внешняя доставка alert/dashboard не имеет `LIVE_VERIFIED`              | Следующая задача B4.5-R2B — XLSX decompression remediation                                        |
...
| Application security      | `BLOCKED`              | Dependency finding, 4 High и Medium organization disclosure закрыты; R2A authorization regression и полный gate stack зелёные                                         | Остаются 3 Medium: XLSX exhaustion, session revocation и notification SSRF       | B4.5-R2B: закрыть XLSX decompression exhaustion и повторить source-to-sink validation                    |
...
Следующая реализационная задача — **B4.5-R2B: XLSX decompression exhaustion**.
B4.5-R2A закрыла organization enumeration/capability disclosure: обычный tenant
с `organization.view` получает `403`, operator сохраняет полный capability
projection; focused unit и PostgreSQL/API regression, полный gate stack и 17/17
browser regression прошли. Проект остаётся `BLOCKED` по Application Security из-за
трёх Medium findings. B4.3 начинается после их source-to-sink remediation и
