...
**Последнее точечное обновление:** 2026-09-03, UI-миграция через commits `a24c734`, `f35367a`, `e25cd97`, `1715b39`, `c5401d3`, `7e99694` и web acceptance closeout `8c9d843`
...
| Автоматические тесты      | `E2E_VERIFIED`         | 2026-09-04: `npm test` — 14/14 Turbo-задач; API 189, schemas 45, api-client 7, Buyer 20, Supplier 21, shared UI 3, Admin 21 и Landing 3 unit-теста. `npm run verify:web` после UI-миграции проходит 17/17 сценариев с локальными PostgreSQL/API и production web bundles | Component coverage части рабочих панелей остаётся ограниченным; live Google/Apple, реальная ЭЦП и ручная accessibility matrix не входят в этот suite | Сохранять `verify:web` как pilot regression gate и добавлять сценарий на каждый новый критический workflow |
| Typecheck и build         | `UNIT_VERIFIED`        | 2026-09-04: `npm run typecheck` — 15/15 Turbo-задач; production builds Admin, Buyer, Supplier и Landing проходят на Next 16.2.11. Buyer, Supplier и Admin production builds дополнительно проверяют initial JS budgets | Полный root build после closeout не повторялся; Next предупреждает о deprecated middleware convention | Сохранять bundle budgets как merge-gate; следующим отдельным срезом профилировать Landing и вторичные Buyer routes |
