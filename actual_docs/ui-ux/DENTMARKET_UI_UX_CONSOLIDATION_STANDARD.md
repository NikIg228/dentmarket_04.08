...
**Последнее решение, 21.09.2026:** после просмотра референсов владелец утвердил
компактный B2B-состав и правила кабинетов (§21). Направление редизайна сохраняется.
Статус: `requirements approved; implementation not started`.
...
## 21. Решение владельца после просмотра — 2026-09-21

Раздел §20 выше — исторический checkpoint до ответа. Владелец просмотрел набор,
обсудил назначение экранов и утвердил итог: «согласен, утверждаю это всё,
приступай, зафиксируй это». Отдельно подтверждена акция «3 + 1»: товар-подарок,
срок, доступность, ограничения, учёт подарка в остатке/резерве и условие в корзине.

Канонические требования сохранены в [Product V2 §22](../product/DENTMARKET_PRODUCT_V2.md#22-утверждённый-состав-b2b-и-правила-кабинетов--2026-09-21),
компоновка — [UI Standard §2.2](UI_UX_IMPLEMENTATION_STANDARD.md#22-утверждённая-структура-кабинетов--21092026),
источники offer data — [ADR 012](../architecture/adr/012-offer-data-authority.md).
Статус: `requirements approved; implementation not started`.

Состав исходных 78 surfaces пересматривается по этим требованиям: часть
объединяется/становится вкладками и модалками, AI/рекомендации/подписки/платная
реклама и публичные информационные/legal-страницы отложены. Это не массовый
ownerApproval=APPROVED для всех исторических макетов. Точные спорные варианты
каталога/токенов и visual/runtime acceptance не подменяются продуктовым согласием.

Исходные HTML/PNG/manifests, generated checklist и доказательства прежней
проверки сохранены без изменений; их PENDING/NOT_RUN/BLOCKED относятся к
историческому набору. Будущая переработка сначала строит карту нового состава,
сохраняя происхождение референсов и работающие feature-компоненты. Текущий
docs-only commit не меняет AppShell, flags, backend, БД и внешние интеграции.
