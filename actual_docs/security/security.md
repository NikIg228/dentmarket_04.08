...
Проверки выбираются для конкретного изменения по
[Development Workflow](../governance/DEVELOPMENT_WORKFLOW.md); чтение этой модели
не запускает полный security scan, live-аудит БД или внешние действия.
При сбое инструмента применяются его ограничения и bounded retry, не обход
permissions/изоляции и не отключение security guards. Документирование policy
не является свежим доказательством отсутствия уязвимостей. Инварианты ниже
сохраняются независимо от ограниченного объёма задачи и экономии запусков.
