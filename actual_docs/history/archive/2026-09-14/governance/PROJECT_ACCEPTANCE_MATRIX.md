> АРХИВ / НЕ ТЕКУЩАЯ ОЧЕРЕДЬ. Снимок до reconciliation 2026-09-14.
> Исходный файл: `actual_docs/governance/PROJECT_ACCEPTANCE_MATRIX.md`.
> Baseline: `3d644963ed72f99a100e180db2d373bc5abeaef9`.
> Исторические статусы, команды и ограничения сохранены; относительные ссылки
> перебазированы к исходным целям. Незакрытые пункты НЕ объявлены выполненными:
> их текущая классификация — в [реестре переноса](../README.md).
> Старые «следующие задачи» ниже не исполняются. Активная очередь —
...
| Backend API               | `E2E_VERIFIED`         | B0.1–B0.6, Flow A, B2.1–B2.4 и B3.1–B3.3 проходят; B4.1–B4.5-R2E и локальный B4.6 baseline проверены; pilot composition содержит 36 modules/224 routes без AI/billing/promotions/trust/recommendations | Длительный staging soak, managed Redis HA/failover и production infrastructure ещё не имеют `LIVE_VERIFIED` | Подготовить live-provider preflight/evidence без заявления готовности до реальных credentials |
...
`apps/buyer-web/app/page.tsx` сначала запрашивает актуальный каталог по `GET /api/catalog/search` через `NEXT_PUBLIC_API_URL`, по 24 товара за страницу. Если API не отвечает за 3,5 секунды или возвращает ошибку, браузер запрашивает те же фильтры у внутреннего `GET /catalog-fallback`. Этот route handler читает локальные JSON-снимки на сервере, поэтому полный резервный каталог не входит в initial client JavaScript. Production build Buyer автоматически запускает `scripts/verify-buyer-bundle.mjs` и отклоняет превышение budget: 24 JS-файла, 1 500 000 raw bytes или 450 000 gzip bytes для `/`.
...
| Документолог Buyer/Supplier         | `E2E_VERIFIED`  | Отдельные `/documents` показывают tenant-safe единый архив с summary, поиском, фильтрами, pagination, detail, участниками, подписями, версиями, accounting status, upload/download; Supplier готовит счёт до оплаты, pack после отгрузки; Flow B2 4/4 и archive-flow 1/1 | Квалифицированная ЭЦП, юридическая валидация шаблонов, ЭДО/ЭСФ/СНТ, 1С и production storage не имеют `LIVE_VERIFIED` | Сохранять B2.4 regression gates; внешние провайдеры и legal sign-off вынести в отдельные задачи |
...
| Buyer V2 UX                         | `E2E_VERIFIED`  | `2bda059` разделяет catalog → product → cart/reprice → checkout → orders на feature-компоненты и использует общие `Dm*`-примитивы; 2026-09-03 `verify:web` подтверждает public catalog/product и создание одиночного/разделённого заказа через реальный локальный API/PostgreSQL | Live production auth/payment и ручная accessibility matrix не проверены | Провести pilot sign-off на реальных ролях и отдельно пройти screen reader/forced-colors/zoom |
| Supplier UX                         | `E2E_VERIFIED`  | Supplier workspace разделён на dashboard/offers/inventory/orders/integrations/compliance/documents; Manrope и общие `Dm*`-контролы применены. `verify:web` подтверждает cabinet smoke, full/partial order confirmation, dispatch, документы, import/publication/rollback и onboarding registration handoff. Performance slice 2026-09-04 ограничивает dashboard семью наборами данных вместо прежних 13, рабочие разделы — 1–4 наборами, выносит неактивные разделы в dynamic chunks и запрещает Fluent icon root barrel. Production bundle `/`: 15 JS-файлов, 1 010 297 raw / 306 886 gzip bytes; browser smoke через gateway: 7/7 dashboard API ответов 200, переход в Orders догружает отдельный chunk и один API resource, console 0 | Реальные 1С/ЭЦП-провайдеры и ручная accessibility matrix не проверены | Закрыть внешние staging/live gates и провести pilot sign-off на реальной роли поставщика |
| Admin UX                            | `E2E_VERIFIED`  | Commits `c5401d3`, `7e99694` и `8c9d843`: shared shell/operator surfaces используют общий `Dm*`-контракт; IA сведена к pilot-сценариям, Admin login/resource lists переведены на shared controls/states и semantic tokens, route-level native controls и hardcoded colors в `apps/admin-web/app` отсутствуют. Production-browser QA 390/768/1024/1280 и `verify:web` 17/17 подтверждают operator publication, Admin smoke, partial API fallbacks, mobile drawer focus/`Escape` и отсутствие mobile overflow. Performance slice 2026-09-04 оставляет overview eager, переводит 16 остальных панелей в dynamic chunks и снижает initial `/` с 26 до 16 JS-файлов, с 1 071 814 до 937 674 raw и с 314 041 до 281 851 gzip bytes; page chunk снижен с 151 336 до 17 676 bytes. Production smoke подтвердил overview и ленивый переход в Organizations с реальными API-данными и console 0 | Live Google/Apple→MFA, screen reader, forced colors, фактический zoom 200/400% и весь operator daily workflow не закрыты | Пройти ручную accessibility matrix и pilot operator sign-off; social/MFA оставить отдельным staging/live gate |
...
- [x] 2026-09-04 `npm run typecheck` проходит: 15/15 Turbo-задач.
- [x] 2026-09-04 `npm test` проходит: 14/14 Turbo-задач; API 189, schemas 45, api-client 7, Buyer 20, Supplier 21, shared UI 3, Admin 21 и Landing 3 теста.
- [x] 2026-09-03 production builds Admin, Buyer, Supplier и Landing проходят на Next 16.2.11.
- [x] 2026-09-03 `npm run verify:web` открывает Landing, Buyer, Supplier и Admin и проходит 17/17 browser-сценариев с локальными API/PostgreSQL без критических ошибок.
...
## Current pilot composition update (2026-09-08)

- [x] Безопасный default и `DEPLOYMENT_PROFILE=pilot` исключают AI, billing,
      promotions, trust/reviews и smart recommendations из Nest graph.
- [x] Geo/address отделён в самостоятельный pilot module без изменения URL.
- [x] `npm run verify:pilot-composition` подтверждает default/pilot/go_live
