...
for (const width of [1280, 390]) {
  test(`product survives registration, email in new tab and password login at ${width}px`, async ({ page, context, browser }) => {
    ...
    await expect(verified.getByRole("heading", { name: "Завершите подключение клиники" })).toBeVisible();
    const form = verified.getByRole("form", { name: "Анкета организации" });
    await form.getByLabel("Контактное лицо").fill("Тестовый владелец");
    await form.getByLabel("Контактный телефон").fill("+77000000000");
    await form.getByLabel("Email организации").fill(email);
    await form.getByLabel("Юридический адрес: город").selectOption({ index: 1 });
    await form.getByLabel("Юридический адрес: улица, дом, помещение").fill("Тестовая улица, дом 1");
    await form.getByRole("checkbox", { name: "Адрес доставки совпадает с юридическим" }).check();
    await form.getByRole("button", { name: "Сохранить анкету и продолжить" }).focus();
    await verified.keyboard.press("Enter");
    await expect(form).toHaveCount(0);
    const savedProfile = await db.organizationProfile.findUniqueOrThrow({ where: { organizationId: registration.organizationId! }, include: { legalAddress: true, deliveryAddress: true } });
