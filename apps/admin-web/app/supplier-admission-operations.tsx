...
import type { SupplierTermsAcceptance, ReviewSupplierAdmissionInput } from "@marketplace/schemas";
...
export function SupplierAdmissionOperations() {
  ...
  return <section className={styles.panel} id="agreements" aria-label="Договоры и допуск поставщиков">
    ...
    {loading ? <LoadingState label="Загружаем принятые договоры" /> : !items.length && !error ? <EmptyState title="Заявок пока нет" description="Поставщики появятся здесь после принятия опубликованных условий." /> : items.map((item) => <article key={item.id} className="mp-stack">
      ...
      {selected === item.id ? <AdmissionReview key={`${item.id}:${item.version}`} item={item} busy={busy} onSubmit={async (input) => {
...
function AdmissionReview({ item, busy, onSubmit }: { item: SupplierTermsAcceptance; busy: boolean; onSubmit: (input: ReviewSupplierAdmissionInput) => Promise<void> }) {
  ...
  return <div className="mp-stack">
    ...
    <div className="mp-inline-actions">
      <DmButton appearance="primary" disabled={busy || reason.trim().length < 3 || !organizationVerified || !representativeVerified} onClick={() => void submit("APPROVED")}>Допустить к работе</DmButton>
