...
const environmentSchema = z
  .object({
    ...
    PUBLIC_CATALOG_ORGANIZATION_ID: z
      .string()
      .uuid()
      .default("00000000-0000-4000-8000-000000000030"),
    ...
    CORS_ORIGINS: z
      .string()
      .default(
        "http://localhost:3000,http://localhost:3001,http://localhost:3002,http://localhost:3003,http://localhost:3010,http://127.0.0.1:3000,http://127.0.0.1:3001,http://127.0.0.1:3002,http://127.0.0.1:3003,http://127.0.0.1:3010",
      ),
  ...
  .superRefine((value, context) => {
    if (value.NODE_ENV === "production" && value.PROCESS_ROLE === "all")
      context.addIssue({
        ...
        path: ["PROCESS_ROLE"],
        message:
          "The all process role is restricted to local development and tests",
      });
    ...
    if (value.NODE_ENV === "production" && value.AUTH_MODE === "development")
      context.addIssue({
        code: "custom",
        path: ["AUTH_MODE"],
        message: "Development identity headers are forbidden in production",
      });
    if (value.AUTH_MODE === "jwt" && !value.JWT_SECRET && !value.JWT_PUBLIC_KEY)
      context.addIssue({
        code: "custom",
        path: ["JWT_PUBLIC_KEY"],
        message: "JWT_PUBLIC_KEY or JWT_SECRET is required in JWT mode",
      });
    if (
      value.SOCIAL_AUTH_ENABLED &&
      !value.JWT_SECRET &&
      ...
      context.addIssue({
        ...
        message:
          "JWT_PRIVATE_KEY or JWT_SECRET is required to issue social sessions",
      });
    if (
      value.SOCIAL_AUTH_ENABLED &&
      !value.GOOGLE_CLIENT_IDS &&
      !value.APPLE_CLIENT_IDS
    )
      context.addIssue({
        code: "custom",
        path: ["GOOGLE_CLIENT_IDS"],
        message: "At least one social client ID is required",
      });
    if (
      value.NODE_ENV === "production" &&
      process.env.SIGNATURE_GATEWAY_URL &&
      !value.SIGNATURE_CALLBACK_SECRET
    )
      context.addIssue({
        code: "custom",
        path: ["SIGNATURE_CALLBACK_SECRET"],
        message:
          "Signed signature callbacks are required when the EDS gateway is configured",
      });
    if (value.NODE_ENV === "production" && value.AV_SCAN_MODE === "disabled")
      context.addIssue({
        code: "custom",
        path: ["AV_SCAN_MODE"],
        message: "File scanning must be enabled in production",
      });
    if (
      value.NODE_ENV === "production" &&
      !value.REDIS_URL &&
      (value.BACKGROUND_QUEUE_ENABLED || value.DEPLOYMENT_PROFILE === "go_live")
    )
      context.addIssue({
        code: "custom",
        path: ["REDIS_URL"],
        message:
          "Redis is required for production queues and shared rate limiting",
      });
    if (
      value.NODE_ENV === "production" &&
      value.DEPLOYMENT_PROFILE === "go_live"
    ) {
      ...
      if (!value.TRUST_PROXY)
        context.addIssue({
          code: "custom",
          path: ["TRUST_PROXY"],
          message: "Trusted reverse proxy mode is required in production",
        });
      if (
        ...
        context.addIssue({
          ...
          path: ["CORS_ORIGINS"],
          message:
            "Production CORS origins must be explicit HTTPS public origins",
        });
      if (!value.JWT_ISSUER || !value.JWT_AUDIENCE)
        context.addIssue({
          code: "custom",
          path: ["JWT_ISSUER"],
          message: "JWT issuer and audience are required in production",
        });
      if (!value.JWT_REQUIRE_MFA)
        context.addIssue({
          code: "custom",
      ...
      if (
        ...
        context.addIssue({
          code: "custom",
          path: ["APP_SECURITY_ENCRYPTION_KEY"],
          message:
            "Independent application and integration encryption keys are required",
        });
      if (!value.MEDIA_SIGNING_SECRET)
        context.addIssue({
          code: "custom",
          path: ["MEDIA_SIGNING_SECRET"],
          message: "Media signing secret is required in production",
        });
      if (
        ...
        !value.S3_SERVER_SIDE_ENCRYPTION
      )
        context.addIssue({
          code: "custom",
      ...
      if (!value.NOTIFICATION_WEBHOOK_SECRET)
        context.addIssue({
          ...
          message:
            "Notification webhook signing secret is required in production",
        });
      if (!value.SENTRY_DSN || !value.OTEL_EXPORTER_OTLP_ENDPOINT)
        context.addIssue({
          code: "custom",
          path: ["SENTRY_DSN"],
          message:
            "Sentry and OTLP telemetry endpoints are required in production",
        });
    }
  });
