...
export class AttributeValuesService {
  ...
  typedData(
    ...
    value: SetAttributeValueInput["value"],
  ): Prisma.ProductAttributeValueUncheckedUpdateInput {
    ...
    if (
      (valueType === "TEXT" || valueType === "LONG_TEXT") &&
      typeof value === "string"
    )
      ...
      data.valueInteger = BigInt(value);
    else if (valueType === "DECIMAL" && typeof value === "number")
      data.valueDecimal = value;
      ...
      data.valueOptionId = value.optionId;
    else if (
      valueType === "MULTI_OPTION" &&
      typeof value === "object" &&
      "optionIds" in value
    )
      data.valueOptionIds = value.optionIds;
    else if (
      valueType === "RANGE" &&
      typeof value === "object" &&
      "min" in value &&
      ...
      data.rangeMin = value.min;
      data.rangeMax = value.max;
    } else if (
      valueType === "NUMBER_WITH_UNIT" &&
      typeof value === "object" &&
      "amount" in value &&
      "unitId" in value
    ) {
      data.valueDecimal = value.amount;
      data.unitId = value.unitId;
    } else
      throw new BadRequestException(
        `Value does not match attribute type ${valueType}`,
      );
    return data;
  }
  ...
  async setProductValue(
    ...
    return this.prisma.$transaction(async (tx) => {
      const stored = await tx.productAttributeValue.upsert({
        where: {
          productId_attributeId: { productId, attributeId: input.attributeId },
        },
        update: data,
        create: {
          productId,
      ...
      await tx.auditLog.create({
        data: {
          ...context,
          action: "catalog.product.attribute.set",
          entityType: "Product",
          entityId: productId,
          after: { attributeId: input.attributeId, value: input.value },
        },
      });
      await tx.outboxEvent.create({
        data: {
          ...
          payload: { productId, attributeId: input.attributeId },
        },
      });
      return stored;
    });
  ...
  async setVariantValue(
    ...
    return this.prisma.$transaction(async (tx) => {
      const stored = await tx.variantAttributeValue.upsert({
        where: {
          variantId_attributeId: { variantId, attributeId: input.attributeId },
        },
        update: data,
        create: {
          variantId,
          attributeId: input.attributeId,
          ...data,
        } as Prisma.VariantAttributeValueUncheckedCreateInput,
      });
      await tx.auditLog.create({
        data: {
          ...context,
          action: "catalog.variant.attribute.set",
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          eventType: "VariantAttributeChanged",
          payload: { variantId, attributeId: input.attributeId },
        },
      });
      return stored;
    });
