...
import type {
  ...
  CreateVariantInput,
  UpdateProductInput,
  UpsertCategoryAttributeRuleInput,
...
export class CatalogService {
  ...
  attributes() {
    return this.prisma.attributeDefinition.findMany({
      include: { group: true, unit: true, options: true },
      orderBy: { nameRu: "asc" },
    });
  }
    ...
    return this.prisma.$transaction(async (tx) => {
      const attribute = await tx.attributeDefinition.create({
        data: {
          ...
          validation: input.validation
            ? (input.validation as Prisma.InputJsonValue)
            : undefined,
        },
      });
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          eventType: "AttributeDefinitionCreated",
  ...
  categories() {
    return this.prisma.category.findMany({
      orderBy: [{ industryId: "asc" }, { path: "asc" }],
    });
  }
  ...
  categoryAttributeRules(categoryId: string) {
    return this.prisma.categoryAttributeRule.findMany({
      where: { categoryId },
      include: {
        attribute: { include: { group: true, unit: true, options: true } },
      },
      orderBy: [{ sortOrder: "asc" }, { attribute: { nameRu: "asc" } }],
    });
  }

  async upsertCategoryAttributeRule(
    categoryId: string,
    input: UpsertCategoryAttributeRuleInput,
    context: ActorContext,
  ) {
    await this.authority.assertPlatformOperator(context);
    const [category, attribute] = await Promise.all([
      this.prisma.category.findUnique({ where: { id: categoryId } }),
      this.prisma.attributeDefinition.findUnique({
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...context,
          action: "catalog.category.attribute_rule.upserted",
          entityType: "Category",
          entityId: categoryId,
          after: rule,
        },
      ...
      await tx.outboxEvent.create({
        data: {
          ...
          eventType: "CategoryAttributeRuleChanged",
          payload: { categoryId, attributeId: input.attributeId },
        },
      });
      return rule;
    });
  ...
  async removeCategoryAttributeRule(
    categoryId: string,
    attributeId: string,
    context: ActorContext,
  ) {
    await this.authority.assertPlatformOperator(context);
    const rule = await this.prisma.categoryAttributeRule.findUnique({
      where: { categoryId_attributeId: { categoryId, attributeId } },
    });
    if (!rule) throw new NotFoundException("Category attribute rule not found");
    return this.prisma.$transaction(async (tx) => {
      await tx.categoryAttributeRule.delete({
        where: { categoryId_attributeId: { categoryId, attributeId } },
      });
      await tx.auditLog.create({
        data: {
          ...context,
          action: "catalog.category.attribute_rule.removed",
          entityType: "Category",
          entityId: categoryId,
          before: rule,
        },
      ...
      await tx.outboxEvent.create({
        data: {
          aggregateType: "Category",
          aggregateId: categoryId,
          eventType: "CategoryAttributeRuleRemoved",
  ...
  async createCategory(input: CreateCategoryInput, context: ActorContext) {
    ...
    const parent = input.parentId
      ? await this.prisma.category.findUnique({ where: { id: input.parentId } })
      : null;
    if (input.parentId && !parent)
      throw new NotFoundException("Parent category not found");
    if (parent && parent.industryId !== input.industryId)
      throw new NotFoundException(
        "Parent category belongs to another industry",
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      const category = await tx.category.create({
        data: { ...input, parentId: input.parentId ?? null, path, depth },
      });
      await tx.auditLog.create({
        data: {
          ...context,
  ...
  async qualityReport() {
    const [
      ...
      this.prisma.$queryRaw<
        Array<{ count: bigint }>
      >`SELECT COUNT(*)::bigint AS count FROM "Product" WHERE "externalMetadata"->>'importedAsCanonicalDraft' = 'true'`,
      this.prisma.$queryRaw<
        Array<{ count: bigint }>
      >`SELECT COUNT(*)::bigint AS count FROM "Product" p WHERE p."externalMetadata"->>'importedAsCanonicalDraft' = 'true' AND NOT EXISTS (SELECT 1 FROM "ProductVariant" v WHERE v."productId" = p.id)`,
      this.prisma.$queryRaw<
        Array<{ count: bigint }>
      >`SELECT COUNT(*)::bigint AS count FROM "Product" p WHERE p."externalMetadata"->>'importedAsCanonicalDraft' = 'true' AND NOT EXISTS (SELECT 1 FROM "ProductCategory" c WHERE c."productId" = p.id)`,
      this.prisma.$queryRaw<
        Array<{ count: bigint }>
      >`SELECT COUNT(*)::bigint AS count FROM "Product" p WHERE p."externalMetadata"->>'importedAsCanonicalDraft' = 'true' AND NOT EXISTS (SELECT 1 FROM "ProductIndustry" i WHERE i."productId" = p.id)`,
      this.prisma.$queryRaw<
        Array<{ count: bigint }>
      >`SELECT COUNT(*)::bigint AS count FROM "Product" p JOIN "ProductSearchDocument" d ON d."productId" = p.id WHERE p."externalMetadata"->>'importedAsCanonicalDraft' = 'true'`,
      this.prisma.$queryRaw<
        Array<{ source: string | null; count: bigint }>
      >`SELECT "externalMetadata"->>'source' AS source, COUNT(*)::bigint AS count FROM "Product" WHERE "externalMetadata"->>'importedAsCanonicalDraft' = 'true' GROUP BY 1 ORDER BY count DESC`,
    ]);
  ...
  async createProduct(input: CreateProductInput, context: ActorContext) {
    await this.authority.assertPlatformOperator(context);
    return this.prisma.$transaction(async (tx) => {
      const product = await tx.product.create({
        data: {
          ...
          searchDocument: {
            create: {
              searchableText: input.canonicalName,
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (result.count === 0)
        throw new ConflictException(
          "Product changed; reload the latest version",
        );
