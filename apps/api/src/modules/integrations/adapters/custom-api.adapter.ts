...
export class CustomApiIntegrationAdapter implements IntegrationAdapter {
  ...
  private url(
    ...
    const baseUrl =
      typeof context.configuration.baseUrl === "string"
        ? context.configuration.baseUrl
        : typeof context.configuration.endpoint === "string"
          ? context.configuration.endpoint
          : "";
    if (!baseUrl || !/^https:\/\//i.test(baseUrl))
      throw new PermanentIntegrationError(
        "CUSTOM_API requires an HTTPS configuration.baseUrl",
      );
    const pathKey = `${operation}Path`;
    const pathValue =
      typeof context.configuration[pathKey] === "string"
        ? (context.configuration[pathKey] as string)
        : `/${operation}`;
    if (!pathValue.startsWith("/") || pathValue.startsWith("//"))
      throw new PermanentIntegrationError(
        `CUSTOM_API ${pathKey} must be a relative path`,
      );
  ...
  private async request(
    ...
    if (!token)
      throw new PermanentIntegrationError(
        "CUSTOM_API accessToken or apiKey is missing",
      );
    ...
    try {
      ...
      const response = await this.outbound.request(
        this.url(context, operation, cursor),
        {
          method,
          timeoutMs: timeout,
      ...
      if (text) {
        try {
          ...
          throw new RetryableIntegrationError(
            "CUSTOM_API returned invalid JSON",
          );
        }
      }
      if (!response.ok) {
        const message =
          typeof payload.message === "string"
            ? payload.message
            : `CUSTOM_API request failed with status ${response.status}`;
      ...
      if (
        ...
        throw new PermanentIntegrationError(
          "CUSTOM_API outbound policy rejected the request",
        );
      if (error instanceof OutboundRequestTimeoutError)
        throw new RetryableIntegrationError("CUSTOM_API request timed out");
      throw new RetryableIntegrationError("CUSTOM_API network request failed");
    }
  }
  ...
  private externalId(row: ApiPayload) {
    return String(row.externalId ?? row.id ?? row.sku ?? "").trim();
  }
  ...
  async testConnection(
    ...
    return {
      ...
      data: { provider: this.provider, health: payload },
    };
  }
  ...
  async pullCatalog(
    ...
    const items: ExternalCatalogItem[] = this.rows(payload)
      .map((value) => {
        ...
        return {
          ...
          supplierSku:
            typeof row.supplierSku === "string"
              ? row.supplierSku
              : typeof row.sku === "string"
                ? row.sku
                : undefined,
          gtin: typeof row.gtin === "string" ? row.gtin : undefined,
  ...
  async pullPrices(
    ...
    const items: ExternalPriceItem[] = this.rows(payload).flatMap((value) => {
      const row = asRecord(value);
      const externalId = this.externalId(row);
      const valueMinor = Number(row.valueMinor ?? row.amountMinor ?? row.price);
      if (!externalId || !Number.isFinite(valueMinor) || valueMinor < 0)
        return [];
      return [
        {
          externalId,
          priceTypeId:
            typeof row.priceTypeId === "string" ? row.priceTypeId : undefined,
          priceTypeName:
            typeof row.priceTypeName === "string"
              ? row.priceTypeName
              : undefined,
          valueMinor,
          currency: typeof row.currency === "string" ? row.currency : "KZT",
          raw: row,
        },
      ];
    });
  ...
  async pullInventory(
    ...
    const items: ExternalInventoryItem[] = this.rows(payload).flatMap(
      (value) => {
        const row = asRecord(value);
        const externalId = this.externalId(row);
        const stock = Number(row.stock ?? row.quantityOnHand ?? 0);
        const reserved = Number(row.reserved ?? row.quantityReserved ?? 0);
        if (
          !externalId ||
        ...
        return [
          {
            externalId,
            externalWarehouseId:
              typeof row.externalWarehouseId === "string"
            ...
            stock,
            reserved,
            available: Number.isFinite(Number(row.available))
  ...
  async exportOrder(
    ...
    return this.post(context, "orders", payload);
  }
  async createReservation(
    context: IntegrationAdapterContext,
    payload: Record<string, unknown>,
  ) {
    return this.post(context, "reservations", payload);
  }
  async releaseReservation(
    context: IntegrationAdapterContext,
    payload: Record<string, unknown>,
  ) {
    return this.post(context, "reservationRelease", payload);
  }
