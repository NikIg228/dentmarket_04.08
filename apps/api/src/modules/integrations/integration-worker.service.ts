...
export class IntegrationWorkerService implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly execution: IntegrationExecutionService,
    private readonly backgroundQueue: BackgroundQueueService,
  ) {}

  onModuleInit() {
    this.backgroundQueue.register("integrations.tick", async () => this.tick());
  }
        ...
        try {
          ...
          const message =
            error instanceof Error
              ? error.message
              : "Unknown integration job failure";
