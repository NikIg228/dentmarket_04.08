...
В репозитории уже есть значительный backend-контур, Prisma-схема, миграции, четыре web-приложения, статический снимок каталога и автоматизированные тесты отдельных правил. При этом на текущей машине не доказаны чистая установка, сборка, применение миграций, запуск API и сквозная покупка. Живые цены, остатки и работа реальных поставщиков также не подтверждены.
...
|---|---|---|---|---|
| История изменений и откат | `BLOCKED` | В корне нет каталога `.git` | Проект скачан ZIP-архивом; нельзя безопасно сравнивать изменения, создавать ветки и откатывать задачи | До любых продуктовых правок инициализировать Git и сделать первый baseline-коммит |
...
| Корзина | `CODED` | Carts, cart items и repricing API, Buyer UI | Нет текущего integration/E2E результата | Проверить один товар, изменение количества и повторный расчёт |
...
- [ ] В каталоге есть Git-репозиторий и baseline-коммит исходного ZIP.
...
Следующая реализационная задача — **Gate 0.1: восстановить Git-базу и чистую установку**. До её завершения нельзя безопасно начинать массовый рефакторинг или Buyer V2: без истории и зелёного baseline невозможно отличить новый дефект от старого и надёжно откатить изменения.
