...
| Backend API               | `INTEGRATION_VERIFIED` | B0.1–B0.6: pilot flow, core contracts, runtime split, PostgreSQL, seed profiles и outbox gates проходят                                            | Сквозные Browser Flow A/B/C ещё не доказаны                                                            | Следующая задача B1.2 — Flow A с UI и DB assertion                                    |
...
| Автоматические тесты      | `UNIT_VERIFIED`        | `pnpm test`: 112 API, 36 schema, 4 api-client, 5 Buyer и 1 Supplier; `pnpm verify:postgres` отдельно проходит                                      | Admin/Landing/UI не имеют unit coverage; Browser Flow A/B/C не пройдены                                | Добавить детерминированные E2E по одному критическому journey                         |
...
| Публичный каталог                   | `CODED`         | API `/catalog/search` и локальный fallback работают по коду; данные валидно читаются     | Нет текущего browser smoke; низкое качество коммерческих данных                     | Использовать как временный источник Buyer V2, не как production truth       |
| Карточка товара                     | `CODED`         | Отдельный Next route, metadata, характеристики, предложения и локальное media            | Страница читает только snapshot; нет live-загрузки по API и единого source of truth | В Buyer V2 перевести на API с корректным offline/fallback режимом           |
...
| Сравнение предложений               | `CODED`         | Compare API и UI/ранжирование присутствуют                                               | В данных только 2 карточки с несколькими продавцами                                 | Сначала подключить минимум 3 поставщика к одним и тем же SKU                |
...
| Корзина                             | `LIVE_VERIFIED` | [x] Построчная проверка старой/новой цены и остатка, принятие изменений, защита checkout | [x] Desktop browser smoke пройден; mobile остаётся частью общего Buyer V2 QA         | Сохранять PostgreSQL reprice gate в `verify:pilot-backend`                  |
| Checkout и разбиение по поставщикам | `CODED`         | Checkout и supplier order API, commerce rules                                            | Нет сквозной транзакции на чистой БД                                                | P0 core-flow после каталога и offer gate                                    |
...
|      P0.4 | Доказать Flow A                      | Реальный сохранённый заказ создаётся из Buyer через API                       |
...
Следующая реализационная задача — **доказать Flow A на pilot fixture: поиск → сравнение → корзина → сохранённый заказ**. Native PostgreSQL baseline, migrations/seed и API search уже подтверждены. Redis, production object storage, ClamAV, browser smoke и внешние интеграции остаются отдельными Gate 0/production-readiness пунктами; они не должны блокировать локальную проверку core-flow, но обязательны перед реальным запуском.
